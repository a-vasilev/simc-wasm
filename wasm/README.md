...
- Node ≥ 21 (for the smoke/CI harness; needs `navigator.hardwareConcurrency`,
  which sizes the thread pool, plus WASM threads + SharedArrayBuffer).
...
### Threading (how many cores the sim uses)

The worker pool is pre-spawned at module load, one worker per logical core:
`-sPTHREAD_POOL_SIZE=navigator.hardwareConcurrency`. To pick the thread count, the
app passes simc's `threads=N` option:

```js
Module.callMain(['/in.simc', 'json2=/out.json', `threads=${n}`]);
```

- **Max parallelism:** omit `threads=` (simc defaults to `hardware_concurrency`)
  or pass `threads=navigator.hardwareConcurrency`.
- **User-configurable:** expose a slider/input and pass the chosen value.
- **Hard rule: `N <= navigator.hardwareConcurrency`.** The build does **not** use
  `-sPROXY_TO_PTHREAD` (see `emscripten.cmake` for why), so the host thread is
  blocked inside `callMain` and cannot service on-demand thread creation. Threads
  beyond the pre-spawned pool would deadlock. Clamp the UI value to the core count.
